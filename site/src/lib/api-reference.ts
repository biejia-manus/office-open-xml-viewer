  ...
  pptx: [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'comments', type: 'boolean | PptxCommentsOptions', def: 'false', desc: 'Show read-only slide comment message icons and built-in margin cards. Message icons are shown by default; pass `markers: false` to hide only the icons. The options object also controls resolved-thread visibility, side, and optional connectors. Theme cards and markers with CSS custom properties or documented classes on the Viewer container.', detailsHref: '/review-ui', detailsLabel: 'Comment UI guide' },
      ...
      methods: [
        ...
        { sig: 'goToComment(slideIndex: number, commentIndex: number, opts?: { behavior?: "auto" | "smooth" }): boolean', desc: 'Reveal and select one entry from presentation.getComments(slideIndex). Returns false when the slide or comment index is invalid. Use it to connect an application-owned comment list to the Scroll Viewer.' },
        ...findMethods('PptxMatchLocation'),
  ...
  docx: [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'comments', type: 'boolean | DocxCommentsOptions', def: 'false', desc: 'Show read-only document comment highlights, message icons, and built-in margin cards. Message icons are shown by default; pass `markers: false` to hide only the icons while keeping highlights and cards. The options object also controls resolved-thread visibility, side, and optional connectors. Theme cards, highlights, and markers with CSS custom properties or documented classes on the Viewer container.', detailsHref: '/review-ui', detailsLabel: 'Comment UI guide' },
      ...
      methods: [
        ...
        { sig: 'goToComment(commentId: string, opts?: { behavior?: "auto" | "smooth" }): Promise<boolean>', desc: 'Resolve a top-level DOCX comment to its rendered page, reveal it, and select the thread. Returns false when the id or rendered anchor is missing. Resolved page locations are cached.' },
        ...findMethods('DocxMatchLocation'),
        { sig: 'setScale(scale: number): void', desc: 'Set the absolute zoom scale at runtime (clamped to zoomMin/zoomMax). Flicker-free. View-only.' },
        { sig: 'relayout(): void', desc: 'Force a re-fit + re-mount of the visible window. Called automatically after load / resize / zoom; use it when the container resizes in a way a ResizeObserver cannot observe (e.g. a late web-font load). Idempotent.' },
        { sig: 'get pageCount(): number', desc: 'Total pages (0 until loaded).' },
  ...
  xlsx: [
    {
      ...
      methods: [
        ...
        { sig: 'goToComment(cellRef: string, options?: XlsxScrollToCellOptions): Promise<boolean>', desc: 'Reveal the cell that owns a current-sheet comment. Returns false when the cell reference does not identify a current-sheet comment.' },
