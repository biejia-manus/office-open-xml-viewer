...
export const chartAddons: readonly ApiAddon[] = [
  {
    ...
    desc: 'Renders the authored OOXML view with one model-space camera and bounded mesh pipeline. Supports the documented cartesian and pie 3-D families and authored bar/column shape meshes. This is a synchronous main-thread renderer, not an interactive orbit API.',
  },
  {
...
const THREE_D = { name: 'threeD', type: 'ChartThreeDRenderer', def: 'undefined', desc: 'Opt-in model-space 3-D chart renderer. Import `threeD` from the separate `@silurus/ooxml/three-d` entry and inject it once. Omit it to use the canonical 2-D fallback and keep the mesh/camera implementation out of the application graph. It renders the view angle authored in OOXML; interactive rotation is not a public API. Main-thread rendering only.', emphasis: 'Opt-in model-space 3-D chart renderer.', detailsHref: '/announcements/v079-chart-rendering-addons#one-3-d-scene-for-axes-and-data', detailsLabel: '3-D scope' };
const REGION_MAP = { name: 'regionMap', type: 'ChartRegionMapRenderer', def: 'undefined', desc: 'Opt-in offline ChartEx Region Map renderer backed by a pinned, public-domain Natural Earth country asset—not Excel or Bing geometry. Import `regionMap` from `@silurus/ooxml/region-map` and inject it once. Unsupported cached or sub-country views fail closed. Main-thread rendering only.', emphasis: 'Opt-in offline ChartEx Region Map renderer', detailsHref: '/announcements/v079-chart-rendering-addons#offline-country-region-maps', detailsLabel: 'Region Map scope' };
