...
export const formatRenderModeGuidance: Readonly<Record<'docx' | 'xlsx' | 'pptx', string>> = {
  docx: 'DOCX keeps page navigation, virtualized scrolling, selection, find, hyperlinks and optional renderers in both modes. Worker mode moves pagination and page paint away from the UI thread.',
  xlsx: 'XLSX keeps sheet tabs, frozen panes, scrolling, selection, find, hyperlinks and optional renderers in both modes. Worker mode paints each requested sheet viewport away from the UI thread.',
  pptx: 'PPTX keeps slide navigation, virtualized scrolling, selection, find, hyperlinks, media playback and optional renderers in both modes. Worker mode moves slide paint away from the UI thread; media controls and overlays remain interactive in the Viewer.',
};
