  ...
  pptx: [
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'goToComment(slideIndex: number, commentIndex: number, opts?: { behavior?: "auto" | "smooth" }): boolean', desc: 'Reveal and highlight one entry from presentation.getComments(slideIndex). Modern anchored comments highlight their target; coordinate comments highlight their authored point. Returns false when either index is invalid.' },
        ...findMethods('PptxMatchLocation'),
  ...
  docx: [
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'goToComment(commentId: string, opts?: { behavior?: "auto" | "smooth" }): Promise<boolean>', desc: 'Resolve a top-level DOCX comment to its rendered page, reveal it, and highlight its authored range. Returns false when the id or rendered anchor is missing. Resolved page locations are cached.' },
        ...findMethods('DocxMatchLocation'),
        { sig: 'setScale(scale: number): void', desc: 'Set the absolute zoom scale at runtime (clamped to zoomMin/zoomMax). Flicker-free. View-only.' },
        { sig: 'relayout(): void', desc: 'Force a re-fit + re-mount of the visible window. Called automatically after load / resize / zoom; use it when the container resizes in a way a ResizeObserver cannot observe (e.g. a late web-font load). Idempotent.' },
        { sig: 'get pageCount(): number', desc: 'Total pages (0 until loaded).' },
  ...
  xlsx: [
    {
      ...
      methods: [
        ...
        { sig: 'goToComment(cellRef: string, options?: XlsxScrollToCellOptions): Promise<boolean>', desc: 'Reveal and select the cell that owns a current-sheet comment. Returns false when the cell reference does not identify a current-sheet comment.' },
