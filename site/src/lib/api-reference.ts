...
const IMAGE_RESOURCES = { name: 'imageResources', type: 'ImageResourceOptions', def: "{ decodedByteBudget: 128 MiB, strategy: 'strict' }", desc: "Decoded-raster policy shared by DOCX, XLSX and PPTX paints. Target-resizable rasters (browser formats and TIFF with its optional codec) preserve native decoding while the complete paint fits. When native ownership would cross an aggregate or per-surface boundary, the renderer uses the current display/DPR target. Strict mode is the default and preserves those requested targets, reporting OoxmlDecodedImageLimitError on an aggregate crossing. Set strategy: 'adaptive' explicitly to apply one uniform quality reduction when the targets still exceed decodedByteBudget; a later zoom requests a sharper cache variant. The budget accepts 4 bytes through 512 MiB; non-resizable sources and encoded-source, per-axis and per-surface hard safety ceilings remain non-disableable.", emphasis: 'Target-resizable rasters (browser formats and TIFF with its optional codec) preserve native decoding while the complete paint fits.', detailsHref: '/errors#decoded-image-limit-error', detailsLabel: 'Safety boundaries' };
