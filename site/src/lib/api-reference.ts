  ...
  docx: [
    {
      ...
      options: [
        ...
        ON_HYPERLINK_CLICK,
        ENABLE_HYPERLINKS,
        { name: 'onPageChange', type: '(index: number, total: number, layoutComplete: boolean) => void', desc: 'Called after a page finishes rendering. During progressive layout, total is the pages available so far and layoutComplete is false.' },
        VIEWER_ON_ERROR,
      ],
      methods: [
        ...
        { sig: 'get layoutComplete(): boolean', desc: 'False while progressive layout is still publishing pages.' },
        { sig: 'whenLayoutComplete(): Promise<void>', desc: 'Wait for the authoritative full layout before operations that require the final page count.' },
        { sig: 'get canvasElement(): HTMLCanvasElement', desc: 'The underlying canvas.' },
