...
| C-BUBBLE-002 | `bubble3D` | Yes | Yes | No | Missing | Group, series, and point values are retained as separate provenance layers, and direct/linked paint remains authoritative for plotted bubble shapes. The application-defined material effect, its point > series > group resolution, and legend composition are intentionally not painted until the bounded Office evidence is complete. |
