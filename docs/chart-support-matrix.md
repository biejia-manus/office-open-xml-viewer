...
| S-STYLE-010 | `dataPoint3D`, `dataPointWireframe`, `floor`, `wall`, `plotArea3D` | Partial | `dataPoint3D` direct/linked solid, gradient, and pattern paint is consumed by classic 3-D bar, pie, line, and area paths with direct no-fill precedence and one paint resolution per datum or series. Structured floor/wall and `plotArea3D` paint is consumed with direct surface formatting authoritative. `dataPointWireframe`, surface thickness, and picture stacking are retained but not yet painted. |
