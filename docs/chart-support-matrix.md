...
| S-STYLE-002 | `chartArea`, `plotArea`, `legend` | Partial | Legend fill/outline fallback, structured fill, and direct `noFill` precedence are consumed in shared 2-D/3-D paths. Plot-area linked structured fills and direct paint precedence are consumed by Cartesian, stock, scatter/bubble, histogram, and Pareto paths. Chart-area styling, plot-area outlines, and the remaining chart families are incomplete. |
