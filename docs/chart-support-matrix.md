...
| S-STYLE-010 | `dataPoint3D`, `dataPointWireframe`, `floor`, `wall`, `plotArea3D` | Partial | `dataPoint3D` direct/linked solid, gradient, and pattern paint is consumed by classic 3-D bar, pie, line, and area paths with direct no-fill precedence and one paint resolution per datum or series. A Surface wireframe consumes fixed-index linked `dataPointWireframe` line paint plus width, dash, cap, and join only when no direct band/series outline needs unmeasured mesh segmentation; relative palettes, compound lines, and those direct cases fail closed. Structured floor/wall and `plotArea3D` paint is consumed with direct surface formatting authoritative. Per-series wireframe formatting, surface thickness, and picture stacking remain unpainted. |
