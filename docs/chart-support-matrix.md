...
| C-STOCK-001 | Stock drop lines, high-low lines, and up/down bars | Yes | Yes | Yes | Partial | Stock `dropLines` now preserve direct/linked line paint and render one category envelope. High-low lines and up/down bars render; remaining high-low/up-down formatting is still under audit. |
...
| S-STYLE-002 | `chartArea`, `plotArea`, `legend` | Partial | Chart-area and legend linked structured fills honor direct paint and `noFill` precedence in shared 2-D/3-D paths. Linked outline color, width, preset dash, cap, and join values fill only omitted direct properties. Plot-area linked fill and the same outline properties are consumed by Cartesian, stock, scatter/bubble, histogram, and Pareto paths. Custom dash sequences and the remaining plot-area chart families are incomplete. |
...
| S-STYLE-011 | `dataPointMarker` and marker layout | Partial | Linked solid marker fill, outline, width, `noFill`, symbol, and size apply to classic marker-bearing series; ChartEx box markers also consume the role. Structured marker fills remain incomplete. |
