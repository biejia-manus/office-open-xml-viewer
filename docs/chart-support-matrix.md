...
| S-STYLE-010 | `dataPoint3D`, `dataPointWireframe`, `floor`, `wall`, `plotArea3D` | Partial | `dataPoint3D` direct/linked solid, gradient, and pattern paint is consumed by classic 3-D bar, pie, line, and area paths with direct no-fill precedence and one paint resolution per datum, series, or observed incoming line segment. A Surface wireframe consumes fixed-index linked `dataPointWireframe` line paint plus width, dash, cap, and join only when no direct band/series outline needs unmeasured mesh segmentation; relative palettes, compound lines, and those direct cases fail closed. Structured floor/wall and `plotArea3D` paint is consumed with direct surface formatting authoritative, and authored thickness uses the shared bounded `CT_Surface` slab projection. Direct 3-D area point body paint, per-series wireframe segmentation, picture stacking, material-dependent slab shading, and grid-rule extension across exterior slab faces remain unpainted. |
