...
| Classic 3-D surface grid-rule continuation | Line3D and Surface3D charts at 0% and 25% floor/wall thickness; distinct authored category/value major-gridline colours, 2 pt widths, and preset dashes; normal camera, `rotY=340`, and `rotX=-20` controls | A category-axis major grid rule continues over the floor and back-wall faces at the same plot-x fraction. A value-axis major grid rule continues over the side- and back-wall faces at the same plot-y fraction. At positive thickness, each rule also continues over every camera-visible outer and joining face belonging to that surface slab. The authored line colour, width, and dash remain unchanged across those segments. This observation does not infer minor-category-gridline semantics or a rule for unmeasured horizontal 3-D chart orientations. |
