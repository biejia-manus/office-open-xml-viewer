...
| Classic 3-D omitted category gridlines and thin axes | Column, stacked-column, horizontal-bar, line, area, box, cylinder, cone, cone-to-max, pyramid, and pyramid-to-max charts; automatic and manual title/axis-label layouts; authored 0.25 pt category, value, and series axes | Current Excel does not synthesize category-depth rays when `catAx/majorGridlines` is absent. Only an authored major- or minor-gridline element contributes that surface geometry. Authored 0.25 pt axis rules remain visibly rasterized; the optional 3-D painter therefore uses the same bounded visible-axis width conversion as the classic 2-D painter for category, value, and series axes, without changing wall, grid, or data-series stroke widths. |
