...
| Classic 3-D planar `CT_Surface` picture options | Excel/PDF probes for floor, side wall, and back wall at 0% and 25% thickness; `applyToFront`/`applyToSides` true and false; stretch with an asymmetric four-color image and visible border; stack and `stackScale` with unit 2; image-without-options and options-without-image controls | On zero-thickness surfaces, stretch maps the complete source rectangle onto the projected face. `applyToFront=false` suppresses the back-wall picture and `applyToSides=false` suppresses the floor/side-wall picture. `stackScale` repeats by value-axis units on back/side walls; Excel ignores `pictureStackUnit` on floor as documented by MS-OE376 §2.1.1543(c). The implementation is limited to this planar stretch/stackScale boundary. Plain `stack`, positive-thickness front/side/end face assignment, reversed value axes, and cropped/tiled image recipes remain fail-closed rather than inferred. |
