...
| Classic 3-D `CT_Surface` picture options | Excel/PDF probes for floor, side wall, and back wall at 0% and 25% thickness; isolated `applyToFront`, `applyToSides`, and `applyToEnd` true/false controls; normal and reversed value axes; back-wall camera controls at rotY 20/340 and rotX 20/-20; stretch with an asymmetric four-color image and visible border; stack and `stackScale` with unit 2; image-without-options and options-without-image controls | On zero-thickness surfaces, stretch maps the complete source rectangle onto the projected face. `applyToFront=false` suppresses the back-wall picture and `applyToSides=false` suppresses the floor/side-wall picture. `stackScale` repeats by value-axis units on planar back/side walls; Excel ignores `pictureStackUnit` on floor as documented by MS-OE376 §2.1.1543(c). Reversing the value axis reverses data/axis placement but leaves the observed stretch and `stackScale` texture ordering unchanged. At 25% thickness, stretch maps the inner face to front, the lateral joining faces to sides, and the alternating end faces to end; omitted face elements remain enabled. The asymmetric quadrants stay screen-upright across the measured positive/negative camera rotations and value-axis directions. The implementation is limited to those observed stretch and planar `stackScale` boundaries. Plain `stack`, positive-thickness `stackScale`, and cropped/tiled image recipes remain fail-closed rather than inferred. |
