...
| Classic 3-D `CT_Surface` picture options | Excel/PDF probes for floor, side wall, and back wall at 0% and 25% thickness; isolated `applyToFront`, `applyToSides`, and `applyToEnd` true/false controls; normal and reversed value axes; back-wall camera controls at rotY 20/340 and rotX 20/-20; stretch with an asymmetric four-color image, positive and negative 25% left source/destination rectangles, and a visible border; stack and `stackScale` with unit 2; image-without-options and options-without-image controls | On zero-thickness surfaces, stretch maps the complete source rectangle onto the projected face. A positive `srcRect` crop removes the authored source edge and maps the remaining intersection across the same full face; a negative `srcRect` outset retains the unavailable source portion as transparent destination space. A positive `fillRect` inset leaves that portion of the face empty and maps the complete source across the remaining projective rectangle; a negative `fillRect` outset expands the destination beyond the face and the existing face clip removes the excess. The same face-local rule holds independently on the measured surfaces and positive-thickness slab faces. `applyToFront=false` suppresses the back-wall picture and `applyToSides=false` suppresses the floor/side-wall picture. `stackScale` repeats by value-axis units on planar back/side walls; Excel ignores `pictureStackUnit` on floor as documented by MS-OE376 §2.1.1543(c). Reversing the value axis reverses data/axis placement but leaves the observed stretch and `stackScale` texture ordering unchanged. At 25% thickness, stretch maps the inner face to front, the lateral joining faces to sides, and the alternating end faces to end; omitted face elements remain enabled. For `stackScale`, repetition continues across front and side faces while an end face receives one complete source because it has no value-axis extent; the floor continues to ignore the stack unit. The asymmetric quadrants stay screen-upright across the measured positive/negative camera rotations and value-axis directions. The implementation is limited to those observed stretch and `stackScale` boundaries. Plain `stack` and tiled image recipes remain fail-closed rather than inferred. |
